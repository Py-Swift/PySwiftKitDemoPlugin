...
export async function activate(context: vscode.ExtensionContext) {
    console.log('🚀 KV to PyClass extension is now active!');
    vscode.window.showInformationMessage('KV to PyClass extension activated!');
    ...
    try {
        ...
        vscode.window.showErrorMessage(
            `Failed to initialize KV to PyClass: ${error instanceof Error ? error.message : String(error)}`
        );
    }
    ...
    if (!hasShownWelcome) {
        vscode.window.showInformationMessage(
            'KV to PyClass extension activated! Open a .kv file to get started.',
            'Show Example'
        ).then(selection => {
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    const generateCommand = vscode.commands.registerCommand(
        'kvToPyClass.generatePythonClass',
        async () => {
    ...
    const toggleAutoGenerateCommand = vscode.commands.registerCommand(
        'kvToPyClass.toggleAutoGenerate',
        () => {
...
function registerLanguageProviders(context: vscode.ExtensionContext) {
    ...
    const renderConfig = vscode.workspace.getConfiguration('kvToPyClass');
...
function setupEventListeners(context: vscode.ExtensionContext) {
    ...
    const configListener = vscode.workspace.onDidChangeConfiguration(event => {
        if (event.affectsConfiguration('kvToPyClass')) {
            const config = vscode.workspace.getConfiguration('kvToPyClass');
            autoGenerateEnabled = config.get<boolean>('autoGenerate', true);
        }
    });
...
export function deactivate() {
    ...
    console.log('KV to PyClass extension deactivated');
}
