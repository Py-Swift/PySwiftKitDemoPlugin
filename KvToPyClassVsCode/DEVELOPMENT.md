...
1. Build the KvToPyClass Swift module to WASM:
   ```bash
...
2. Copy WASM artifacts to extension:
   ```bash
   cp build/KvToPyClass.wasm KvToPyClassVsCode/wasm/
   cp build/runtime.js KvToPyClassVsCode/wasm/
   ```
...
- `kvToPyClass.autoGenerate`: Enable auto-generation on file changes
- `kvToPyClass.debounceDelay`: Delay in ms before generating (prevents excessive updates)
- `kvToPyClass.previewOnSide`: Open preview in separate column vs current column

## Commands

- `kvToPyClass.generatePythonClass`: Generate Python class and open in new file
- `kvToPyClass.showPreview`: Open/focus the preview panel
- `kvToPyClass.toggleAutoGenerate`: Toggle auto-generation on/off
