...
async function initBrowser(_options) {
    ...
    if (!module) {
        module = fetch(new URL("PythonToSwiftDemo.wasm.gz", import.meta.url)).then(async r => { const ds = new DecompressionStream("gzip"); return new Response(r.body.pipeThrough(ds), { headers: { "Content-Type": "application/wasm" } }); })
    }
